...
public class Player extends MainTrainer
{
    ...
    public void walk(){ 
        ...
        if(keyIs("down")&& scrollOnDirection(DOWN)){
            setImage("P-front.png");
            setLocation(getX(), getY() + 5);
            lastMove = 1;
        }
        else if(keyIs("up")&& scrollOnDirection(UP)){
            setImage("P-back.png");
            setLocation(getX(), getY() - 5);
            lastMove = 2;
        }
        else if(keyIs("right")&& scrollOnDirection(RIGHT)){
            setImage("p-right.png");
            setLocation(getX() + 5, getY());   
            lastMove = 3;
        }
        else if (keyIs("left")&& scrollOnDirection(LEFT)){
            setImage("p-left.png");
            setLocation(getX() - 5, getY());
            lastMove = 4;
        }
