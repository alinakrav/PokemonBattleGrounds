...
public class Selection extends Actor
{
    ...
    public void act() 
    {
        ...
        if(keyIs("down") && grid[0][0] instanceof PartyTag && gridIndex[0] > 0 && gridIndex[1] == grid[gridIndex[0]].length - 1) {
        ...
        else if(keyIs("down") && gridIndex[1] < grid[gridIndex[0]].length - 1)
            setLocation(grid[gridIndex[0]][++gridIndex[1]].getX(), grid[gridIndex[0]][gridIndex[1]].getY()); 
        else if(keyIs("up") && grid[0][0] instanceof PartyTag && gridIndex[0] < grid.length - 1 && gridIndex[1] == grid[gridIndex[0]].length - 1) {
            setLocation(grid[++gridIndex[0]][grid[gridIndex[0]].length - 1].getX(), grid[gridIndex[0]][grid[gridIndex[0]].length - 1].getY());
            gridIndex[1] = grid[gridIndex[0]].length - 1;
        }
        ...
        if(grid[0][0] instanceof Item) {
            if((canPress || (((Item)currentItem()).name).equals("Close")) && keyIs("enter")) // if the selection is on bag items, it can only press according to canPress boolean
                selectCurrent();
        }
        else if(keyIs("enter")) // if selection is not for bag items, then it will always press
            selectCurrent();
    }
