...
public class ItemsList extends Actor
{
        ...
        if(init) { 
            ...
            if(!items.isEmpty()) { // if the list is of bag items
                for(int i = 0; i < items.size(); i++) {// create objects for all the items 
                    world.addObject(items.get(i), items.get(i).getX(), items.get(i).getY());
                }
                ...
                world.removeObject(((Battle)getWorld()).player);
    ...
    public void prepareInScrolling() {
        if(init) { 
            ...
            if(!items.isEmpty()) { // if list is for bag
                ...
                world.removeObject(((ScrollingWorld)getWorld()).player);
    ...
    public void select(Item choice) {
        ...
        if(world instanceof Battle)
            ((Battle)world).player.useItem(chosen, false);
        else
            ((ScrollingWorld)world).player.useItem(chosen, false);
